...
在IOT这边干了3年多，正好赶上当时数据库比较火，运气比较好，就单纯依靠相对比较扎实的基础和对技术的热情，0基础跳槽去了一家数据库创业公司，我所在的团队是查询团队，主要负责优化器执行器相关的优化工作，比重上的话执行器更多一些。主要包括执行引擎的重构迁移、各种算子优化，包括Sort算子、Window算子、Exchange算子、参与优化器的迭代工作，包括支持更多类型的子查询、Plan枚举以及支持Profile相关的功能。
