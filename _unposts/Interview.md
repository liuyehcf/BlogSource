...
本硕就读于北京邮电大学的通信工程专业，然后在硕士阶段的业余时间自学计算机相关课程。毕业之后，就职于阿里巴巴物联网事业部，参与物联网平台的建设，物联网平台涵盖许多业务领域，整个平台又可以细分为许多子系统，包括设备中台（负责设备的连接、管理和赋能）、边缘计算、边缘智能、以及各种垂直行业线，包括工业、农业、城市、园区、地产等等。我在里面主要负责偏Infra相关的工作，主要包括：隧道服务（提供访问私网设备服务的能力）、设备规则中心（管理设备联动规则），边缘一体机操作系统镜像构建平台，所谓边缘一体机就是一些具有一定计算能力和AI能力的网关设备，负责设备的接入以及数据的处理、以及云边协同，不同的业务场景需要用到不同的软件、不同型号的机器，所以需要有一个平台来创建对应的系统安装包，然后交给主机厂商进行量化生产。细节不展开了，简历里面都有介绍。

在IOT这边干了3年多，正好赶上当时数据库比较火，运气比较好，就单纯依靠相对比较扎实的基础和对技术的热情，0基础跳槽去了一家数据库创业公司，我所在的团队是查询团队，主要负责优化器执行器相关的优化工作，比重上的话执行器更多一些。主要包括执行引擎的重构迁移、各种算子优化，包括Sort算子、Window算子、Exchange算子、参与优化器的迭代工作，包括支持更多类型的子查询、Plan枚举、子查询优化，主外键优化，另外还负责管理Profile相关的功能的迭代和演进工作。
