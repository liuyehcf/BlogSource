...
* {% raw %}$FIRST(X)${% endraw %}：可以从{% raw %}$X${% endraw %}推导出的所有**串首终结符**构成的集合
...
* {% raw %}$FOLLOW(A)${% endraw %}：可能在某个句型中紧跟在{% raw %}$A${% endraw %}后边的**终结符**{% raw %}$a${% endraw %}的集合
...
* 忽略输入中的一些符号，直到输入中出现由设计者选定的同步词法单元(synchronizing token)集合中的某个词法单元
    ...
    * 例如可以把**{% raw %}$FOLLOW(A)${% endraw %}中的所有终结符**放入非终结符A的同步记号集合
