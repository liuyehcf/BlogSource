...
* `grub`启动后，选择对应的启动菜单项，`grub`会通过自带文件系统驱动，定位到对应的`eki`文件
* `grub`会尝试加载`eki`文件（`eki`文件需要满足`grub`多协议引导头的格式要求），这些定义在`imginithead.asm`中，`imginithead.asm`的逻辑如下：
    * `grub header`
    * `grub`校验成功后，会调用`_start`（`grub`控制跳转到`_start`处）
        * 跳转到`_entry`
            * 关闭中断
            * 将`GDT`加载到`GDTR`寄存器
            * 然后进入`_32bits_mode`
                * 清理寄存器
                * 设置栈顶
                * 调用C编写的`inithead_entry`（`inithead.c`）
                    * 初始化光标，清屏
                    * 从`eki`文件内部，找到`initldrsve.bin`文件，并分别拷贝到内存的指定物理地址
                    * 从`eki`文件内部，找到`initldrkrl.bin`文件，并分别拷贝到内存的指定物理地址
                * `jmp 0x200000`，该位置存放了`initldrkrl.bin`，于是开始执行`initldrkrl.bin`的内容（`initldr32.asm`）
                    * 将`GDT`加载到`GDTR`寄存器
                    * 将`IDT`加载到`IDTR`寄存器
                    * 跳转到`_32bits_mode`
                        * 初始寄存器
                        * 初始化栈
                        * 调用C编写的`ldrkrl_entry`（`ldrkrlentry.c`）
                            * 初始化光标，清屏
                            * 调用`init_bstartparm`收集机器参数（`bstartparm.c`）
                                * 初始化`machbstart_t`
                                * 各类初始化函数，填充`machbstart_t`的内容
                    * `jmp 0x2000000`，跳转到该地址，继续执行
