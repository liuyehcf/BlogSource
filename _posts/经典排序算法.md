...
bool get_m_matrix(const std::vector<int32_t>& A, const int32_t i, const std::vector<int32_t>& B, const int32_t j) {
    if (i < 0) {
        return false;
    } else if (i >= A.size()) {
        return true;
...
std::pair<int32_t, int32_t> diagnoal_intersection(std::vector<int32_t>& A, std::vector<int32_t>& B, int32_t p_i,
    ...
    while (true) {
        ...
        bool m_1 = get_m_matrix(A, i - 1, B, j - 1);
        bool m_2 = get_m_matrix(A, i - 1, B, j);
        bool m_3 = get_m_matrix(A, i, B, j - 1);
        bool m_4 = get_m_matrix(A, i, B, j);

        bool has_true = m_1 || m_2 || m_3 || m_4;
        bool has_false = !m_1 || !m_2 || !m_3 || !m_4;
...
void merge(const std::vector<int32_t>& A, const int32_t a_start, const std::vector<int32_t>& B, const int32_t b_start,
           std::vector<int32_t>& S, int32_t s_start, int32_t length) {
    int32_t i = a_start;
    int32_t j = b_start;
    int32_t k = s_start;
...
void parallel_merge(std::vector<int32_t>& A, std::vector<int32_t>& B, std::vector<int32_t>& S, int32_t p) {
    ...
    for (int i = 0; i <= p; ++i) {
        ...
        merge(A, a_start, B, b_start, S, s_start, length);
    }
}
