...
1. MySQL数据库事务的隔离级别
    > 1. __Read uncommitted(读未提交)__：
    >   * 事务读数据时不加锁
    >   * 事务写数据的时候(写操作时才加锁而不是事务一开始就加锁)加__行级共享锁__，__事务结束释放__
    >   * 行级别的共享锁不会对读产生影响，但是可以防止两个同时的写操作
    > 1. __Read committed(读已提交)__：
    >   * 事务读数据的时候(读操作时才加锁而不是事务一开始就加锁)加__行级共享锁__，__读完释放__
    >   * 事务写数据的时候(写操作时才加锁而不是事务一开始就加锁)加__行级独占锁__，__事务结束释放__
    >   * 由于事务写操作加上独占锁，因此事务写操作时，读操作不能进行，因此，不能读到事务的未提交数据，避免了脏读问题
    >   * __但是由于读操作的锁加在读上面，而不是加在事务之上，所以，在同一事务的两次读操作之间可以插入其他事务的写操作，所以可能发生不可重复读的问题__
    > 1. __Repeatable read(可重复读)__：
    >   * 事务读数据的时候(读操作时才加锁而不是事务一开始就加锁)加__行级共享锁__，__事务结束释放__
    >   * 事务写数据的时候(写操作时才加锁而不是事务一开始就加锁)加__行级独占锁__，__事务结束释放__
    > 1. __Serializable(串行化)__：
...
1. 长连接的实现
    > 在HTTP1.0和HTTP1.1协议中都有对长连接的支持。__其中HTTP1.0需要在request的请求头中增加"Connection：keep-alive" header才能够支持，而HTTP1.1默认支持__
    > __http1.0请求与服务端的交互过程__：
...
1. Unicode/UTF-8/UTF-16的区别
    > 简单来说：
    > 1. Unicode 是「字符集」
    > 1. UTF-8/UTF-16是「编码规则」

    > 其中：
    > 1. 字符集：为每一个「字符」分配一个唯一的 ID（学名为码位 / 码点 / Code Point）
    > 1. 编码规则：将「码位」转换为字节序列的规则（编码/解码 可以理解为 加密/解密 的过程）
