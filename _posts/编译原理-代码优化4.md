...
{% raw %}$IN[B]${% endraw %}：在基本块B入口处的支配结点集合

{% raw %}$OUT[B]${% endraw %}：在基本块B出口处的支配结点集合
...
1. 寻找{% raw %}$L${% endraw %}中__只有一次定值__的变量{% raw %}$k${% endraw %}，它具有下面的形式：{% raw %}$k=c^{\\'} \times j + d^{\\'}${% endraw %}。其中{% raw %}$c^{\\'}${% endraw %}和{% raw %}$d^{\\'}${% endraw %}是常量，{% raw %}$j${% endraw %}是__基本的__或__非基本的__归纳变量
    * 如果{% raw %}$j${% endraw %}__是基本归纳变量__，那么{% raw %}$k${% endraw %}属于{% raw %}$j${% endraw %}族。{% raw %}$k${% endraw %}对应的三元组可以通过其定值语句确定
    * 如果{% raw %}$j${% endraw %}__不是基本归纳变量__，假设其属于{% raw %}$i${% endraw %}族，{% raw %}$k${% endraw %}的三元组可以通过{% raw %}$j${% endraw %}的三元组和{% raw %}$k${% endraw %}的定值语句来计算（将{% raw %}$j = c \times i+d${% endraw %}代入{% raw %}$k=c^{\\'} \times j + d^{\\'}${% endraw %}），此时我们还要求：
