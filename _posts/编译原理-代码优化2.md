...
{% raw %}$IN[s]${% endraw %}：语句{% raw %}$s${% endraw %}之前的数据流值

{% raw %}$OUT[s]${% endraw %}：语句{% raw %}$s${% endraw %}之后的数据流值

{% raw %}$f_s${% endraw %}：语句{% raw %}$s${% endraw %}的__传递函数__(transfer function)

* 一个赋值语句{% raw %}$s${% endraw %}之前和之后的数据流值的关系
* 传递函数的两种风格
    * 信息沿执行路径前向传播(前向数据流问题){% raw %}$$OUT[s] = f_s (IN[s])$${% endraw %}
    * 信息沿执行路径逆向传播(逆向数据流问题){% raw %}$$IN[s] = f_s (OUT[s])$${% endraw %}
...
{% raw %}$IN[B]${% endraw %}：紧靠基本块B__之前__的数据流值

{% raw %}$OUT[B]${% endraw %}：紧随基本块B__之后__的数据流值
...
* 如果__存在一条__从紧跟在定值{% raw %}$d${% endraw %}后面的点到达某一程序点{% raw %}$p${% endraw %}的路径，而且在此路径上{% raw %}$d${% endraw %}没有被“杀死”（如果在此路径上有对变量{% raw %}$x${% endraw %}的其它定值{% raw %}$d^{\\'}${% endraw %}，则称变量{% raw %}$x${% endraw %}被这个定值{% raw %}$d^{\\'}${% endraw %}__“杀死”__了），则称__定值__{% raw %}$d${% endraw %}__到达__程序点{% raw %}$p${% endraw %}
...
{% raw %}$f_d${% endraw %}：定值{% raw %}$d: u = v + w${% endraw %}的传递函数
...
{% raw %}$IN[B]${% endraw %}：到达流图中基本块B的入口处的定值的集合

{% raw %}$OUT[B]${% endraw %}：到达流图中基本块B的出口处的定值的集合
