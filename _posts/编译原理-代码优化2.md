...
1. {% raw %}$f_s${% endraw %}：语句{% raw %}$s${% endraw %}的**传递函数**(transfer function)
...
1. {% raw %}$IN[B]${% endraw %}：紧靠基本块B**之前**的数据流值
1. {% raw %}$OUT[B]${% endraw %}：紧随基本块B**之后**的数据流值
1. {% raw %}$f_B${% endraw %}：基本块{% raw %}$B${% endraw %}的**传递函数**(transfer function)
...
* {% raw %}$f_d(x) = gen_d \cup (x-kill_d)${% endraw %}，即**生成-杀死形式**
* {% raw %}$gen_d${% endraw %}：由语句{% raw %}$d${% endraw %}生成的定值的集合{% raw %}$gen_d =\{ d \}${% endraw %}
* {% raw %}$killd${% endraw %}：由语句{% raw %}$d${% endraw %}杀死的定值的集合（程序中**所有**其它对{% raw %}$u${% endraw %}的定值）
