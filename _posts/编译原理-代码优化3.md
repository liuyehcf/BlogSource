...
* 对于变量{% raw %}$x${% endraw %}和程序点{% raw %}$p${% endraw %}，如果在流图中沿着从{% raw %}$p${% endraw %}开始的**某条**路径会引用变量{% raw %}$x${% endraw %}在{% raw %}$p${% endraw %}点的值，则称变量{% raw %}$x${% endraw %}在点{% raw %}$p${% endraw %}是**活跃(live)**的，否则称变量{% raw %}$x${% endraw %}在点{% raw %}$p${% endraw %}**不活跃(dead)**
...
* {% raw %}$def_B${% endraw %}：在基本块B中定值，但是**定值前**在B中**没有被引用**的变量的集合
* {% raw %}$use_B${% endraw %}：在基本块B中引用，但是**引用前**在B中**没有被定值**的变量集合
