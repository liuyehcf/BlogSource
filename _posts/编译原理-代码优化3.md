...
* 如果B中{% raw %}$x${% endraw %}的定值{% raw %}$d${% endraw %}之后有{% raw %}$x${% endraw %}的第一个定值{% raw %}$d^{\\'}${% endraw %}，则{% raw %}$d${% endraw %}和{% raw %}$d^{\\'}${% endraw %}之间{% raw %}$x${% endraw %}的所有引用构成{% raw %}$d${% endraw %}的{% raw %}$du${% endraw %}链
