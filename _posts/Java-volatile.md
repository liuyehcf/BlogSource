...
__当写一个volatile变量时，JVM会把该线程对应的本地内存中的共享变量(不仅仅是该volatile变量，而是该线程缓存的所有共享变量)值刷新到主内存__
...
__当读一个volatile变量时，JVM会把该线程对应的本地内存(不仅仅是该volatile对应的内存，而是该线程缓存的所有变量的内存)置为无效。线程接下来将从主内存中读取共享变量__
