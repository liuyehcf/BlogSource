...
Here are some of the special characters that commonly appear in C++ name mangling and their meanings:
...
void func(double y) {
    std::cout << "foo(double): " << y << std::endl;
}
} // namespace Outer::Inner

int main() {
    Outer::Inner::Foo<int> myInt(42);
    Outer::Inner::Foo<double> myDouble(3.14159);

    myInt.print();
    myDouble.print();

    Outer::Inner::func(42);
    Outer::Inner::func(3.14159);
...
```sh
gcc -o main main.cpp -lstdc++ -std=gnu++17 -g -ggdb -O0 && nm main
```

Explanation:

1. The mangled name `_ZN5Outer5Inner3FooIdE5printEv` can be broken down as follows:
    1. `_ZN`: This is a common prefix in mangled names, indicating that what follows is the mangled name of a C++ symbol, such as a function or class.
    1. `5Outer`: This part represents the class or namespace name. The `5` indicates that the name has 5 characters, and `Outer` is the actual name.
    1. `5Inner`: This part represents the class or namespace name. The `5` indicates that the name has 5 characters, and `Inner` is the actual name.
    1. `3FooIdE`: This part represents the function name and its template specialization:
        * `3Foo` is the name of the function. The `3` indicates that the name `Foo` has 3 characters.
        * `IdE` represents the template specialization. Specifically, `I` denotes the start of the template argument list, and `d` indicates that the template argument is of type `double`. `E` denotes the end of the template argument list.
    1. `5printEv`: This part represents the name of a function:
        * `5print` is the name of the function. The `5` indicates that the name `print` has 5 characters.
        * `E`: between name and parameter list.
        * `v`: `v` typically represents the function has no parameters.
1. The mangled name `_ZN5Outer5Inner5func1Ed` can be broken down as follows:
    1. `5func1Ed`: This part represents the name of a member function:
        * `5func1` is the name of the function. The `5` indicates that the name `func1` has 5 characters.
        * `E`: between name and parameter list.
        * `d` represents the argument type, which is `double`.
1. The mangled name `_ZN5Outer5Inner5func2Eii` can be broken down as follows:
    1. `5func2Eii`: This part represents the name of a member function:
        * `5func2` is the name of the function. The `5` indicates that the name `func2` has 5 characters.
        * `E`: between name and parameter list.
        * `ii` represents the argument types, which are `int`, `int`.
