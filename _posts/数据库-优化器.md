...
初始状态。显然，`Child`与`CurBox`存在关联

```sql
SELECT Q1.name
FROM Dept Q1, Temp1 Q2
...
* `CiBox`与`CurBox`存在关联，但这个相关性可以通过`Eq Join`解除
    * 对于`CiBox`，每个`building`只有一条数据
    ```sql
    SELECT Q1.name
    FROM Supp Q1 JOIN CiBox Q2
    ON Q1.building = Q2.building
    WHERE Q1.num_emps > Q2.$1;
    ```

* `Child`与`CurBox`存在关联，但由于`Magic_1`的存在，`Child`与`DcoBox`也存在关联。因此，关联关系就被下推了
* 于是`CurBox`的相关性就被解除了

```sql
SELECT Q1.name
FROM Supp Q1, CiBox Q2
WHERE Q1.num_emps > Q2.$1;
