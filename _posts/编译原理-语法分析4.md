...
{% raw %}$SLR${% endraw %}分析表与{% raw %}$LR(0)${% endraw %}分析表的区别
...
1. 令{% raw %}$I_i${% endraw %}对应状态{% raw %}$i${% endraw %}。状态{% raw %}$i${% endraw %}的语法分析动作按照下面的方法决定：
    ...
    * {% raw %}$\textbf{if}\; A \to \alpha \cdot \in I_i 且 A \ne S^{\\'} \;\textbf{then}\; \;\textbf{for}\; \forall a \in FOLLOW(A) \;\textbf{do}\; ACTION[i, a] = rj${% endraw %}（{% raw %}$j${% endraw %}是产生式{% raw %}$A \to \alpha${% endraw %}的编号）    __*（与{% raw %}$LR(0)${% endraw %}分析表算法的唯一区别）*__
...
* {% raw %}$S \to L \;\cdot = R\;\;${% endraw %}表示当下一个符号是`=`时，采取移入动作
* {% raw %}$R \to L \cdot\;\;${% endraw %}表示当下一个符号是`=`或`$`时，采取归约动作
...
* 在特定位置，{% raw %}$A${% endraw %}的后继符集合是{% raw %}$FOLLOW(A)${% endraw %}的子集，__一般情况下是真子集__（例如上图中，非终结符{% raw %}$R${% endraw %}的后继符一个是`=`，另一个是`{% raw %}$`，而${% endraw %}FOLLOW(R) = \{ =, \{% raw %}$ \}${% endraw %}）

## 2.2 规范{% raw %}$LR(1)${% endraw %}项目

将一般形式为{% raw %}$[A \to \alpha \cdot \beta, a]${% endraw %}的项称为{% raw %}$LR(1)${% endraw %}项，其中{% raw %}$A \to \alpha \beta${% endraw %}是一个产生式，{% raw %}$a${% endraw %}是一个终结符(这里将`{% raw %}$`视为一个特殊的终结符)。它表示在当前状态下，${% endraw %}A$后面必须紧跟的终结符，__称为该项的展望符(lookahead)__
...
## 2.7 为文法{% raw %}$G^{\\'}${% endraw %}构造{% raw %}$LR(1)${% endraw %}项集族
...
1. 构造{% raw %}$G^{\\'}${% endraw %}的规范{% raw %}$LR(1)${% endraw %}项集族{% raw %}$C = \{ I_0, I_1,... , I_n \}${% endraw %}
1. 根据{% raw %}$I_i${% endraw %}构造状态{% raw %}$i${% endraw %}。状态{% raw %}$i${% endraw %}的语法分析动作按照下面的方法决定：
    ...
    * {% raw %}$\textbf{if}\; [A \to \alpha \cdot, a] \in I_i 且 A \ne S^{\\'} \;\textbf{then}\; ACTION[i, a] = rj${% endraw %}（{% raw %}$j${% endraw %}是产生式{% raw %}$A \to \alpha${% endraw %}的编号）  
    * {% raw %}$\textbf{if}\; [S^{\\'} \to S \cdot, \$] \in I_i \;\textbf{then}\; ACTION [ i,\$] = acc${% endraw %}
1. 没有定义的所有条目都设置为“error”
...
{% raw %}$LR(1)${% endraw %}分析法会产生许多__同心状态__，如下图中红色标注之处
