...
**生成的头文件`org_liuyehcf_jni_JniDemo.h`如下：**
...
第二种方式：**修改`org_liuyehcf_jni_JniDemo.h`，并且将`jni.h`以及`jni_md.h`文件拷贝到`org_liuyehcf_jni_JniDemo.h`文件所在的目录中**
...
* `-o`：指定生成的动态库文件的名称，在macOS环境下，**动态库的命名格式为`libXXX.jnilib`，相当于linux下的`libXXX.so`**
...
* **注意`-Djava.library.path`参数指定的是libXXX.jnilib所在的目录**
