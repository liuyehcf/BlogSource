...
1. 可以看成是__将输入串{% raw %}$w${% endraw %}归约为文法开始符号{% raw %}$S${% endraw %}__的过程
...
首先，{% raw %}$LR${% endraw %}文法(Knuth, 1963) 是最大的、可以构造出相应__移入-归约语法分析器__的文法类
...
__方法__：初始时，语法分析器栈中的内容为初始状态{% raw %}$s_0${% endraw %}，输入缓冲区中的内容为{% raw %}$w\$${% endraw %}。然后，语法分析器执行下面的程序：
