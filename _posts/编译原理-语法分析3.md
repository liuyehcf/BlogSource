...
1. 可以看成是**将输入串{% raw %}$w${% endraw %}归约为文法开始符号{% raw %}$S${% endraw %}**的过程
...
* {% raw %}$B \to \cdot${% endraw %}与{% raw %}$T \to \cdot${% endraw %}会产生**归约/归约冲突**，即不知道使用哪个进行归约
* {% raw %}$B \to \cdot${% endraw %}与{% raw %}$T \to a \cdot Bd${% endraw %}会产生**移进/归约冲突**，即不知道该移进还是该归约
