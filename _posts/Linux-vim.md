...
1. 按下i进入编辑模式
    ...
    * __此时除了Esc这个按键之外，其他的按键都可以视作一般的输入了__
...
* `[n]x`：连续向后删除n个字符
* __`dd`：删除光标所在一整行__
* __`dw`：删除光标所在的字符到光标所在单词的最后一个字符__
...
* __`y$`：复制光标所在那个字符到该行最后一个字符的所有数据(包括光标所指向的字符)__
