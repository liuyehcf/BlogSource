...
**y是`等效`被删除的节点（或者说，y是被删除的节点或者即将移动到被删除节点的节点）**

* 当z最多只有一个孩子时，y就是被删除的节点
* 当z有两个孩子时，y就是即将移动到被删除节点的节点（替代原来的z，于是原来的y节点被删除了）

**x是`将要`移动到y节点的节点（即x代表占有y原来位置的节点）**
...
**此外，还有另一个版本（避免讨论，即不用讨论(y.parent==z)），两个版本等价**
...
1. **删除最终等效为删除一个最多只有一个孩子的节点**
    ...
    * 当被删除节点z有两个孩子，那么找到该节点z的后继节点y，**此时y节点必然最多只有一个右孩子**，于是将其右孩子y.right transplant到到y节点处以删除y节点，然后再将y节点移动到z节点处，并保持z节点原来的颜色，那么等价于删除y节点
