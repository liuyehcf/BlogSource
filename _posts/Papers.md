...
| [Everything You Always Wanted to Know About Compiled and Vectorized Queries But Were Afraid to Ask](/resources/paper/Everything-You-Always-Wanted-to-Know-About-Compiled-and-Vectorized-Queries-But-Were-Afraid-to-Ask.pdf) | <li>`Execution`</li> | <li>Vectorization(pull base) and data-centric code generation(push base) are both good</li><li>Vectorization is better when executing calculation-heavy queries, because it is good at hiding cache miss latency</li><li>two constraints of vectorization: It can (i) only work on one data type2 and it (ii) must process multiple tuples</li> | 👀 |  |
